...
func TestFuncAIR(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.Int{Value: 10})
	require.NoError(t, err)
	require.Equal(t, vvm.UndefinedValue, ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncAR(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, vvm.UndefinedValue, ret)
	_, err = funcCall(uf, vvm.TrueValue)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncARI(t *testing.T) {
	...
	ret, err := funcCall(uf)
	require.NoError(t, err)
	require.Equal(t, &vvm.Int{Value: 10}, ret)
	_, err = funcCall(uf, vvm.TrueValue)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncARE(t *testing.T) {
	...
	ret, err := funcCall(uf)
	require.NoError(t, err)
	require.Equal(t, vvm.TrueValue, ret)
	uf = stdlib.FuncARE(func() error { return errors.New("some error") })
	ret, err = funcCall(uf)
	require.NoError(t, err)
	require.Equal(t, &vvm.Error{
		Value: &vvm.String{Value: "some error"},
	}, ret)
	_, err = funcCall(uf, vvm.TrueValue)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncARIsE(t *testing.T) {
	...
	ret, err := funcCall(uf)
	require.NoError(t, err)
	require.Equal(t, array(&vvm.Int{Value: 1},
		&vvm.Int{Value: 2}, &vvm.Int{Value: 3}), ret)
	uf = stdlib.FuncARIsE(func() ([]int, error) {
...
func TestFuncARS(t *testing.T) {
	...
	ret, err := funcCall(uf)
	require.NoError(t, err)
	require.Equal(t, &vvm.String{Value: "foo"}, ret)
	_, err = funcCall(uf, vvm.TrueValue)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncARSs(t *testing.T) {
	...
	ret, err := funcCall(uf)
	require.NoError(t, err)
	require.Equal(t, array(&vvm.String{Value: "foo"},
		&vvm.String{Value: "bar"}), ret)
	_, err = funcCall(uf, vvm.TrueValue)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncASRE(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.String{Value: "foo"})
	require.NoError(t, err)
	require.Equal(t, vvm.TrueValue, ret)
...
func TestFuncASRSs(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.String{Value: "foo"})
	require.NoError(t, err)
	require.Equal(t, array(&vvm.String{Value: "foo"}), ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncASI64RE(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.String{Value: "foo"}, &vvm.Int{Value: 5})
	require.NoError(t, err)
	require.Equal(t, vvm.TrueValue, ret)
	...
	require.NoError(t, err)
	require.Equal(t,
		&vvm.Error{Value: &vvm.String{Value: "some error"}}, ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncAIIRE(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.Int{Value: 5}, &vvm.Int{Value: 7})
	require.NoError(t, err)
	require.Equal(t, vvm.TrueValue, ret)
...
func TestFuncASSRE(t *testing.T) {
	...
	require.Equal(t,
		&vvm.Error{Value: &vvm.String{Value: "some error"}}, ret)
	_, err = funcCall(uf, &vvm.String{Value: "foo"})
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncASsRS(t *testing.T) {
	...
	ret, err := funcCall(uf, array(&vvm.String{Value: "foo"},
		&vvm.String{Value: "bar"}), &vvm.String{Value: " "})
	require.NoError(t, err)
	require.Equal(t, &vvm.String{Value: "foo bar"}, ret)
	_, err = funcCall(uf, &vvm.String{Value: "foo"})
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncARF(t *testing.T) {
	...
	ret, err := funcCall(uf)
	require.NoError(t, err)
	require.Equal(t, &vvm.Float{Value: 10.0}, ret)
	_, err = funcCall(uf, vvm.TrueValue)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncAFRF(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.Float{Value: 10.0})
	require.NoError(t, err)
	require.Equal(t, &vvm.Float{Value: 10.0}, ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
	_, err = funcCall(uf, vvm.TrueValue, vvm.TrueValue)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncAIRF(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.Int{Value: 10.0})
	require.NoError(t, err)
	require.Equal(t, &vvm.Float{Value: 10.0}, ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
...
func TestFuncAFRI(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.Float{Value: 10.5})
	require.NoError(t, err)
	require.Equal(t, &vvm.Int{Value: 10}, ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
...
func TestFuncAFRB(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.Float{Value: 0.1})
	require.NoError(t, err)
	require.Equal(t, vvm.TrueValue, ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
...
func TestFuncAFFRF(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.Float{Value: 10.0},
		&vvm.Float{Value: 20.0})
	require.NoError(t, err)
	require.Equal(t, &vvm.Float{Value: 30.0}, ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
	_, err = funcCall(uf, vvm.TrueValue)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
...
func TestFuncASIRS(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.String{Value: "ab"}, &vvm.Int{Value: 2})
	require.NoError(t, err)
	require.Equal(t, &vvm.String{Value: "abab"}, ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
	_, err = funcCall(uf, vvm.TrueValue)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
...
func TestFuncAIFRF(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.Int{Value: 10}, &vvm.Float{Value: 20.0})
	require.NoError(t, err)
	require.Equal(t, &vvm.Float{Value: 30.0}, ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
...
func TestFuncAFIRF(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.Float{Value: 10.0}, &vvm.Int{Value: 20})
	require.NoError(t, err)
	require.Equal(t, &vvm.Float{Value: 30.0}, ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
...
func TestFuncASSIRSs(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.String{Value: "foo"},
		&vvm.String{Value: "bar"}, &vvm.Int{Value: 5})
	require.NoError(t, err)
	require.Equal(t, array(&vvm.String{Value: "foo"},
		&vvm.String{Value: "bar"}, &vvm.String{Value: "5"}), ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncARYE(t *testing.T) {
	...
	ret, err := funcCall(uf)
	require.NoError(t, err)
	require.Equal(t, &vvm.Bytes{Value: []byte("foo bar")}, ret)
	uf = stdlib.FuncARYE(func() ([]byte, error) {
		return nil, errors.New("some error")
	})
	ret, err = funcCall(uf)
...
func TestFuncASRIE(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.String{Value: "foo"})
	require.NoError(t, err)
	require.Equal(t, &vvm.Int{Value: 5}, ret)
...
func TestFuncAYRIE(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.Bytes{Value: []byte("foo")})
	require.NoError(t, err)
	require.Equal(t, &vvm.Int{Value: 5}, ret)
...
func TestFuncASSRI(t *testing.T) {
	...
	ret, err := funcCall(uf,
		&vvm.String{Value: "foo"}, &vvm.String{Value: "bar"})
	require.NoError(t, err)
	require.Equal(t, &vvm.Int{Value: 6}, ret)
	_, err = funcCall(uf, &vvm.String{Value: "foo"})
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncASSRS(t *testing.T) {
	...
	ret, err := funcCall(uf,
		&vvm.String{Value: "foo"}, &vvm.String{Value: "bar"})
	require.NoError(t, err)
	require.Equal(t, &vvm.String{Value: "foobar"}, ret)
	_, err = funcCall(uf, &vvm.String{Value: "foo"})
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncASSRB(t *testing.T) {
	...
	ret, err := funcCall(uf,
		&vvm.String{Value: "123"}, &vvm.String{Value: "12"})
	require.NoError(t, err)
	require.Equal(t, vvm.TrueValue, ret)
	_, err = funcCall(uf, &vvm.String{Value: "foo"})
	require.Equal(t, vvm.ErrWrongNumArguments, err)
...
func TestFuncAIRS(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.Int{Value: 55})
	require.NoError(t, err)
	require.Equal(t, &vvm.String{Value: "55"}, ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncAIRIs(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.Int{Value: 55})
	require.NoError(t, err)
	require.Equal(t, array(&vvm.Int{Value: 55}, &vvm.Int{Value: 55}), ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncARI64(t *testing.T) {
	...
	ret, err := funcCall(uf)
	require.NoError(t, err)
	require.Equal(t, &vvm.Int{Value: 55}, ret)
	_, err = funcCall(uf, &vvm.Int{Value: 55})
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncASsSRS(t *testing.T) {
	...
	ret, err := funcCall(uf,
		array(&vvm.String{Value: "abc"}, &vvm.String{Value: "def"}),
		&vvm.String{Value: "-"})
	require.NoError(t, err)
	require.Equal(t, &vvm.String{Value: "abc-def"}, ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func TestFuncAI64RI64(t *testing.T) {
	...
	ret, err := funcCall(uf, &vvm.Int{Value: 55})
	require.NoError(t, err)
	require.Equal(t, &vvm.Int{Value: 110}, ret)
	_, err = funcCall(uf)
	require.Equal(t, vvm.ErrWrongNumArguments, err)
}
...
func array(elements ...vvm.Object) *vvm.Array {
	return &vvm.Array{Value: elements}
}
