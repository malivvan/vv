...
func Equal(t *testing.T, expected, actual interface{}, msg ...interface{}) {
	...
	case *vvm.Symbol:
		if !equalSymbol(expected, actual.(*vvm.Symbol)) {
			failExpectedActual(t, expected, actual, msg...)
		}
	case parser.Pos:
	...
	case []vvm.Object:
		equalObjectSlice(t, expected, actual.([]vvm.Object), msg...)
	case *vvm.Int:
		Equal(t, expected.Value, actual.(*vvm.Int).Value, msg...)
	case *vvm.Float:
		Equal(t, expected.Value, actual.(*vvm.Float).Value, msg...)
	case *vvm.String:
		Equal(t, expected.Value, actual.(*vvm.String).Value, msg...)
	case *vvm.Char:
		Equal(t, expected.Value, actual.(*vvm.Char).Value, msg...)
	case *vvm.Bool:
		if expected != actual {
			failExpectedActual(t, expected, actual, msg...)
		}
	case *vvm.Array:
		equalObjectSlice(t, expected.Value,
			actual.(*vvm.Array).Value, msg...)
	case *vvm.ImmutableArray:
		equalObjectSlice(t, expected.Value,
			actual.(*vvm.ImmutableArray).Value, msg...)
	case *vvm.Bytes:
		if !bytes.Equal(expected.Value, actual.(*vvm.Bytes).Value) {
			failExpectedActual(t, string(expected.Value),
				string(actual.(*vvm.Bytes).Value), msg...)
		}
	case *vvm.Map:
		equalObjectMap(t, expected.Value,
			actual.(*vvm.Map).Value, msg...)
	case *vvm.ImmutableMap:
		equalObjectMap(t, expected.Value,
			actual.(*vvm.ImmutableMap).Value, msg...)
	case *vvm.CompiledFunction:
		equalCompiledFunction(t, expected,
			actual.(*vvm.CompiledFunction), msg...)
	case *vvm.Undefined:
		if expected != actual {
			failExpectedActual(t, expected, actual, msg...)
		}
	case *vvm.Error:
		Equal(t, expected.Value, actual.(*vvm.Error).Value, msg...)
	case vvm.Object:
		if !expected.Equals(actual.(vvm.Object)) {
			failExpectedActual(t, expected, actual, msg...)
		}
...
func equalSymbol(a, b *vvm.Symbol) bool {
...
func equalObjectMap(
	t *testing.T,
	expected, actual map[string]vvm.Object,
	msg ...interface{},
) {
	Equal(t, len(expected), len(actual), msg...)
...
func equalCompiledFunction(
	...
	expectedT := expected.(*vvm.CompiledFunction)
	actualT := actual.(*vvm.CompiledFunction)
	Equal(t,
		vvm.FormatInstructions(expectedT.Instructions, 0),
		vvm.FormatInstructions(actualT.Instructions, 0), msg...)
}
