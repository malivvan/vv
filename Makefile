...
test: generate lint
	@((go test -json -cover ./pkg/... || (echo ERROR && exit 1)) ; (GODEBUG=randseednop=0 go test -json -race -cover ./vvm/...|| (echo ERROR && exit 1))) | gotestsum --format $(TEST_FORMAT) --format-hide-empty-pkg --hide-summary skipped --raw-command -- cat
