...
func (c *Conn) write(buf *bytesutil.ByteBuffer) error {
...
func (c *Conn) writeNoWait(buf *bytesutil.ByteBuffer) error {
	_, err := c.preparePendingWrite(buf, false)
	return err
}

func (c *Conn) preparePendingWrite(buf *bytesutil.ByteBuffer, wait bool) (*pendingWrite, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Conn) getMessageHandler() MessageHandler {
	if c.Message == nil {
...
func (c *Conn) call(seq uint32, data []byte) error {
	msg := acquireMessage(c, seq, data)
	defer releaseMessage(msg)
	return c.getMessageHandler().HandleMessage(msg)
}
